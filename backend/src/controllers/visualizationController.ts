...
import { getCacheService } from '../services/cacheServiceProvider';
...
export const getVisualizations = async (req: Request, res: Response) => {
  try {
    ...
    if (cachedAnalysis) {
      ...
      const analysisResult = JSON.parse(cachedAnalysis as string);
    ...
    const analysisService = new BackendAnalysisService(githubToken);
