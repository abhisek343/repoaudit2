...
exports.LLMService = exports.LLMProcessingError = exports.LLMError = void 0;
...
class LLMProcessingError extends LLMError {
    details;
    constructor(message, details) {
...
exports.LLMProcessingError = LLMProcessingError;
class LLMService {
    openai;
    googleAI;
    claude;
    ...
    static QUOTA_COOLDOWN_MS = 60 * 60 * 1000; // 1 hour cooldown
    constructor(config) {
        this.config = {
            ...config,
        ...
        try {
            switch (this.config.provider) {
                case 'openai':
                    ...
                    break;
                case 'gemini':
                    this.googleAI = new genai_1.GoogleGenAI({ apiKey: this.config.apiKey });
                    break;
                case 'claude':
    ...
    async enhanceMermaidDiagram(diagramCode, fileInfo) {
        if (!this.isConfigured()) {
...
`;
    ...
    getDefaultModel(provider) {
        if (provider === 'openai')
            return 'gpt-4o';
        if (provider === 'gemini')
            return 'gemini-2.0-flash'; // Updated default Gemini model
        if (provider === 'claude')
    ...
    async generateText(prompt, maxTokens = 1000) {
        ...
        try {
            switch (this.config.provider) {
                ...
                case 'gemini': {
                    if (!this.googleAI)
                        throw new LLMError('Google AI (Gemini) not initialized.');
                    let retryCount = 0;
                    const maxRetries = 4;
                ...
                case 'claude': {
                    if (!this.claude)
                        throw new LLMError('Anthropic Claude not initialized');
    ...
    async generateSummary(codebaseContext) {
        try {
    ...
    createSummaryPrompt(codebaseContext) {
        return `
...
Return ONLY the JSON object.`;
    ...
    async generateExecutiveSummary(repoData) {
        if (!this.isConfigured())
            return "LLM not configured. Summary unavailable.";
        const structuredSummary = await this.generateSummary(`Repository: ${repoData.name}, Description: ${repoData.description}, Language: ${repoData.language}, Stars: ${repoData.stars}`);
        return structuredSummary.summary;
    }
    async explainFunction(functionCode, fileName, language, context) {
        if (!this.isConfigured())
            return "LLM not configured. Function explanation unavailable.";
        const prompt = `
...
Keep it technical but accessible to developers. Aim for 200-250 words. Return only the explanation text.`;
    ...
    async analyzeCodeComplexity(fileInfo) {
        if (!this.isConfigured() || !fileInfo.content)
            return "LLM not configured or file content missing. Complexity analysis unavailable.";
        const prompt = `
...
Be specific and actionable. Aim for 150-200 words. Return only the analysis text.`;
    ...
    async analyzeArchitecture(files, languages) {
        if (!this.isConfigured())
            return "LLM not configured. Architecture analysis unavailable.";
...
Provide a comprehensive and detailed analysis. Aim for 400-500 words. Return only the analysis text.`;
    ...
    async generateSecurityAnalysis(files, repoName) {
        ...
        const relevantFiles = files.filter(f => f.path.includes('config') ||
            f.path.includes('util') ||
            f.path.includes('service') ||
            f.path.includes('auth') ||
            f.name.match(/\.(?:env|pem|key|secret|token|conf|cfg|xml|json|yaml|yml)/i) ||
            f.name.match(/^(?:config|settings|secret|credential|auth|token)/i)).slice(0, 15).map(f => f.path);
        const prompt = `
...
Focus on actionable insights. Return only the analysis text.`;
    ...
    async analyzeAlgorithmicComplexity(content, fileName) {
        if (!this.isConfigured())
            return null;
        const prompt = `
...
Return ONLY a JSON array of roadmap items.`;
    ...
    async performSecurityAnalysis(securityData) {
        if (!this.isConfigured()) {
    ...
    async assessCodeQuality(qualityMetrics) {
        if (!this.isConfigured()) {
