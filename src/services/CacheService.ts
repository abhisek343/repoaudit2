export class CacheService {
  private cache = new Map<string, { data: unknown; timestamp: number }>(); // data: any -> unknown
  private defaultTTL: number; // Default Time To Live in milliseconds

  constructor(defaultTTL: number = 5 * 60 * 1000) { // Default to 5 minutes
    this.defaultTTL = defaultTTL;
  }

  async get<T = unknown>(key: string): Promise<T | null> { // T = any -> unknown
  ...
  async set<T = unknown>(key: string, data: T, ttl?: number): Promise<void> { // T = any -> unknown
